...
local library =
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Storm99999/whitelistkeys/main/octolib.lua"))(
...
game.RunService.RenderStepped:Connect(function()
        for _,vls in next, game.Players:GetPlayers() do
            if vls.Character and configTable.Visuals.Chams then
                if aimhook:FindFirstChild(vls.Name) then
                   
                else
                    if configTable.Visuals.Chams then
                        if aimhook:FindFirstChild(vls.Name) then return end
                        local highlight = Instance.new("Highlight", aimhook)
            ...
            if not configTable.Visuals.Chams then
                if aimhook:FindFirstChild(vls.Name) then
        	       aimhook[vls.Name]:Destroy()     
        	   end  
            end
        end
	...
	game.Players.PlayerAdded:Connect(function(v)
	    while task.wait() do
	        if v.Character and configTable.Visuals.Chams then
	           if aimhook:FindFirstChild(v.Name) then return end
	...
	game.Players.PlayerRemoving:Connect(function(v)
	    ...
	    if aimhook:FindFirstChild(v.Name) then
	        aimhook[v.Name]:Destroy()     
	   end
	end)
